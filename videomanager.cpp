...
bool VideoManager::validateFileExists() {

}


bool VideoManager::setOptions() {
    
}


int VideoManager::populateFormatContext(AVFormatContext* format_ctx, const char* input_file) {
...
int VideoManager::getAudioStream(const char* input_file) {
    for (unsigned int i = 0; i < this->input_format_ctx->nb_streams; i++) {
        if (this->input_format_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
...
const AVCodec* VideoManager::getAudioCodec() {
    const AVCodec* audio_codec = avcodec_find_decoder(this->input_format_ctx->streams[this->audio_stream_idx]->codecpar->codec_id);
    if (!audio_codec) {
...
int VideoManager::copyAudioCodecParams() {
    ...
    if (avcodec_parameters_to_context(this->audio_codec_ctx, this->input_format_ctx->streams[this->audio_stream_idx]->codecpar) < 0) {
...
int VideoManager::createOutputContext() {
    if (avformat_alloc_output_context2(&this->output_format_ctx, nullptr, nullptr, this->output_file) < 0) {
